...
class Inventory(pygame.sprite.Sprite):
  ...
  def update(self):
    hpPerc = self.advent.hp / self.advent.maxHp * 18/4
    manaPerc = self.advent.mana / self.advent.maxMana * 18/4
    xpPerc = self.advent.xp / self.advent.xpTillNext * 18/4
    self.hpBar = pygame.Surface((tileSize*hpPerc, tileSize/2))
    self.manaBar = pygame.Surface((tileSize*manaPerc, tileSize/2))
    self.xpBar = pygame.Surface((tileSize*xpPerc, tileSize/2))
    self.hpBar.fill((225, 0, 0))
    self.manaBar.fill((88, 88, 217))
    self.xpBar.fill((25, 170, 25))
    self.image.fill((40, 40, 40))
